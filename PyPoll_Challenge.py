...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in county_votes:
       ...
       turnout_vote = county_votes[county_name]
     
    # 6c: Calculate the percent of total votes for the county.
    county_percentage = float(county_votes) / float(total_votes) * 100

    # 6d: Print the county results to the terminal.
    print(f"{county_name}: {county_percentage:.1f}% ({turnout_votes:,})\n")

    # 6e: Save the county votes to a text file.
    file_to_save = os.path.join("Resources", election_analysis.txt)

    # 6f: Write a decision statement to determine the winning county and get its vote count.
    if (county_votes > winning_count) and (vote_percentage > winning_percentage):
